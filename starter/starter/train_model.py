...
cat_metrics_list = compute_model_metrics_by_slice(model=model, X=test, cat_features=cat_features,
    label="salary", encoder=encoder, lb=lb)

with open(MODEL_METRICS_REPORT_PATH, "w") as model_report_f:
    model_report_writer = csv.DictWriter(model_report_f,
        fieldnames=['category_feature', "category", "precision", "recall", "fbeta", "num_elements"])
